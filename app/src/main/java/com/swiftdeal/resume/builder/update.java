...
public class update extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(com.swiftdeal.resume.builder.R.layout.update_main);
        Toolbar toolbar = (Toolbar) findViewById(com.swiftdeal.resume.builder.R.id.toolbar);
        setSupportActionBar(toolbar);

        if (savedInstanceState == null) {
            Bundle arguments = new Bundle();
            arguments.putParcelable(updatefragment.UPDATE_URI, getIntent().getData());

            updatefragment fragment = new updatefragment();
            fragment.setArguments(arguments);
            getSupportFragmentManager().beginTransaction().add(com.swiftdeal.resume.builder.R.id.resume_update_container, fragment).commit();
        }
